...
namespace
{
	...
	// Escapes what may not appear raw in HTML text or in a quoted attribute value. A surface is named by
	// whoever wrote the hive file, and that name reaches the link below by way of the path it is mounted at.
...
GraphHiveSurfaceHtmlMap::GraphHiveSurfaceHtmlMap(HttpServerBase& httpServer, GraphHiveSurface& surface,
	std::string path) :
	GraphHiveSurfaceHttpMap(httpServer, surface, path), _surface(&surface)
{
	surface.addListener(*this);
}

GraphHiveSurfaceHtmlMap::~GraphHiveSurfaceHtmlMap()
{
	...
	_getSurface().removeListener(*this);
...
void GraphHiveSurfaceHtmlMap::setPollInterval(unsigned pollIntervalMs)
{
	{ SYNC(_lock)
...
void GraphHiveSurfaceHtmlMap::setStructurePath(std::string path)
{
	{ SYNC(_lock)
...
void GraphHiveSurfaceHtmlMap::setChatCapability(AgenticHarness::Capability capability)
{
	{ SYNC(_lock)
...
std::string GraphHiveSurfaceHtmlMap::_getBasePath()
{
...
void GraphHiveSurfaceHtmlMap::_renderPageTemplate(const std::string& pageTemplate, HttpResponse& response)
{
	...
	{ SYNC(_lock)
		...
		structurePath = _structurePath;
	}

	if(title.empty()) title = "Graph Hive Surface";

	// The element being there at all is what tells the page there is a structure to go to, so a hive with no
	// graph view surface mounted gets a page with nothing on it to press rather than a dead link.
	std::string structureMarkup;

	if(!structurePath.empty())
	{
		structureMarkup = "<a id=\"structureLink\" href=\"" + htmlEscape(structurePath) +
			"\" title=\"View this hive's graph structure\">Structure</a>";
	}
	...
	replacePlaceholder(page, "%STRUCTURE_MARKUP%", structureMarkup);
...
void GraphHiveSurfaceHtmlMap::_serveData(HttpRequest& request, HttpResponse& response)
{
...
void GraphHiveSurfaceHtmlMap::__serveRevision(HttpResponse& response)
{
	unsigned revision;
...
void GraphHiveSurfaceHtmlMap::__servePoke(HttpRequest& request, HttpResponse& response)
{
	...
	// Whether this reaches the graph at all is the surface's business: one that is read only discards it.
	_getSurface().poke(nodeId, GraphPoke(type, data, chunkId));

	response.setContentType("application/json");
	response.setBody("{\"ok\":true}");
}

void GraphHiveSurfaceHtmlMap::__serveChat(HttpRequest& request, HttpResponse& response)
{
...
void GraphHiveSurfaceHtmlMap::__serveChatMessage(HttpRequest& request, HttpResponse& response)
{
...
void GraphHiveSurfaceHtmlMap::__serveChatContexts(HttpResponse& response)
{
	std::vector<GraphHiveSurface::ChatContext> contexts = _getSurface().getChatContexts();
...
void GraphHiveSurfaceHtmlMap::__serveChatContextRemove(HttpRequest& request, HttpResponse& response)
{
	...
	try
	{
		_getSurface().removeChatContext(contextId);
	}
...
bool GraphHiveSurfaceHtmlMap::__startChatThread()
{
	{ SYNC(_lock)
...
unsigned GraphHiveSurfaceHtmlMap::__queueChatMessage(std::string prompt, bool newContext, unsigned contextId)
{
	unsigned messageId;
...
bool GraphHiveSurfaceHtmlMap::__takeNextChatMessage(ChatMessage& message)
{
	bool taken = false;
...
void GraphHiveSurfaceHtmlMap::__completeChatMessage(unsigned messageId, unsigned contextId, std::string reply,
	std::string error)
{
	_chatMessagesCond.lockMutex();
...
void GraphHiveSurfaceHtmlMap::threadEntry()
{
	while(!_getQuit())
	{
		ChatMessage message;
		...
		try
		{
			reply = _getSurface().chat(message.prompt, capability, message.newContext, contextId);
		}
		catch(GraphException& exception)
		{
...
void GraphHiveSurfaceHtmlMap::_quitRequested()
{
...
void GraphHiveSurfaceHtmlMap::hiveSurfaceChanged(EventEmitter<GraphHiveSurfaceListener>& emitter)
{
	{ SYNC(_lock)
...
void GraphHiveSurfaceHtmlMap::populateEventListenerHandle(CastHandle<GraphHiveSurfaceListener>& handle)
{
